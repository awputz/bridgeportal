...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Contacts auth action: ${action} for user: ${user.id}`);

    if (action === 'get-auth-url') {
      const redirectUri = `${SUPABASE_URL}/functions/v1/google-contacts-auth`;
    ...
    if (action === 'disconnect') {
      await supabase
        ...
        .update({ 
          ...
          contacts_access_token: null,
          contacts_refresh_token: null
        })
        .eq('user_id', user.id);
...
async function handleOAuthCallback(code: string, userId: string) {
  console.log(`Handling Contacts OAuth callback for user: ${userId}`);
  ...
  try {
    ...
    return createHtmlResponse('success', 'Google Contacts connected successfully!');
  } catch (error: unknown) {
