...
import { Copy, Users, Calendar, Building2, Home, Briefcase, UserPlus, CheckCircle2, Clock, Mail, Phone, ExternalLink, FileText, Link as LinkIcon, Check, TrendingUp, QrCode, Download, ChevronDown } from "lucide-react";
...
import { useMyIntakeLink, useIntakeSubmissions, useIntakeStats, useUpdateSubmission, useConvertToContact, IntakeSubmission } from "@/hooks/useIntake";
import { cn } from "@/lib/utils";
import { IntakeStatsCard, IntakeFilters, IntakeSubmissionRow, IntakeSubmissionRowSkeleton, IntakeEmptyState } from "@/components/portal/intake";
...
export default function Intake() {
  ...
  const filteredSubmissions = useMemo(() => {
    return submissions.filter(s => s.client_name.toLowerCase().includes(searchQuery.toLowerCase()) || s.client_email.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const handleFilterChange = (type: 'division' | 'status' | 'search', value: string) => {
    setCurrentPage(1);
    if (type === 'division') setSelectedDivision(value);else if (type === 'status') setSelectedStatus(value);else setSearchQuery(value);
  };

  // Calculate conversion rate
  const conversionRate = stats && stats.total > 0 ? Math.round(submissions.filter(s => s.status === 'converted').length / stats.total * 100) : 0;
  return <div className="flex-1 overflow-auto">
        ...
        <CardContent className="space-y-5">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <Button onClick={handleCopyLink} size="lg" className="shadow-sm">
                {copied ? <>
              ...
              <Button variant="outline" size="lg" onClick={() => window.open(universalIntakeUrl, '_blank')}>
          ...
          {!linkLoading && personalIntakeUrl && <Collapsible className="pt-4 border-t border-border/50">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <IntakeStatsCard title="Total Submissions" value={stats?.total || 0} icon={Users} variant="default" isLoading={statsLoading} />
        <IntakeStatsCard title="This Week" value={stats?.thisWeek || 0} icon={Calendar} trend={{
          value: stats?.thisWeek || 0,
          label: "this week"
        }} variant="primary" isLoading={statsLoading} />
        <IntakeStatsCard title="Awaiting Contact" value={stats?.newSubmissions || 0} icon={Clock} variant="warning" isLoading={statsLoading} />
        <IntakeStatsCard title="Conversion Rate" value={conversionRate} icon={TrendingUp} variant="success" isLoading={statsLoading} suffix="%" />
      </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          {filteredSubmissions.length > 0 && <Badge variant="secondary" className="text-sm px-3 py-1">
              {filteredSubmissions.length} total
            </Badge>}
        </div>

        {/* Filters */}
        <IntakeFilters searchQuery={searchQuery} onSearchChange={v => handleFilterChange('search', v)} selectedDivision={selectedDivision} onDivisionChange={v => handleFilterChange('division', v)} selectedStatus={selectedStatus} onStatusChange={v => handleFilterChange('status', v)} />

        {/* Submissions Table */}
        <Card className="overflow-hidden">
          <CardContent className="p-0">
            {submissionsLoading ? <Table>
                <TableHeader>
                ...
                <TableBody>
                  {[...Array(5)].map((_, i) => <IntakeSubmissionRowSkeleton key={i} />)}
                </TableBody>
              </Table> : filteredSubmissions.length === 0 ? <IntakeEmptyState type="submissions" /> : <>
                <Table>
                  <TableHeader>
                    <TableRow className="bg-muted/30 hover:bg-muted/30">
                      <TableHead className="w-[280px] h-14 font-semibold">Client</TableHead>
                  ...
                  <TableBody>
                    {paginatedSubmissions.map(submission => <IntakeSubmissionRow key={submission.id} submission={submission} onView={() => setSelectedSubmission(submission)} onMarkContacted={() => updateSubmission.mutate({
                    id: submission.id,
                    status: "contacted",
                ...
                {/* Pagination */}
                {totalPages > 1 && <div className="flex items-center justify-between px-6 py-4 border-t border-border">
      ...
      {selectedSubmission && <SubmissionDetailDialog submission={selectedSubmission} onClose={() => setSelectedSubmission(null)} />}

      {/* QR Code Dialog */}
      {showQR && <QRCodeDialog url={universalIntakeUrl} onClose={() => setShowQR(false)} />}
      </div>
    </div>;
}
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="flex flex-col items-center gap-6 py-6">
          <div className="p-4 bg-white rounded-xl shadow-inner">
            <img src={qrCodeUrl} alt="QR Code for intake form" className="w-48 h-48" />
          </div>
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader className="space-y-4">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className={cn("flex h-14 w-14 items-center justify-center rounded-full bg-primary/10 text-primary text-xl font-semibold")}>
            ...
            <Badge variant="outline" className={cn("capitalize text-sm", statusColors[submission.status])}>
              {submission.status}
            </Badge>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {submission.client_phone && <Button variant="outline" size="sm" asChild>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {submission.client_phone && <div className="p-4 rounded-xl bg-muted/50 border border-border/50">
                  <p className="text-xs text-muted-foreground mb-1">Phone</p>
                  <p className="font-medium">{submission.client_phone}</p>
                </div>}
              {submission.client_company && <div className="p-4 rounded-xl bg-muted/50 border border-border/50">
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-3">
              {Object.entries(submission.criteria).map(([key, value]) => <div key={key} className="p-4 rounded-xl bg-muted/50 border border-border/50">
          ...
          {/* Notes */}
          {submission.notes && <div className="space-y-4">
              <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">
                Additional Notes
              </h3>
              <div className="p-4 rounded-xl bg-muted/50 border border-border/50">
          ...
          {submission.converted_contact_id && <div className="flex items-center gap-3 p-4 rounded-xl bg-green-500/10 border border-green-500/20">
              <CheckCircle2 className="h-5 w-5 text-green-600 shrink-0" />
              <span className="text-sm font-medium text-green-700">Converted to contact</span>
              <Link to={`/portal/crm/contacts/${submission.converted_contact_id}`} className="text-sm text-primary hover:underline ml-auto flex items-center gap-1">
                View Contact
        ...
        <DialogFooter className="gap-2 sm:gap-2">
          <Button variant="outline" onClick={onClose}>Close</Button>
          {submission.status === "new" && <Button variant="secondary" onClick={() => {
          ...
          {!submission.converted_contact_id && <Button onClick={() => {
