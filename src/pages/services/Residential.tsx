...
const processSteps = [{
  step: "01",
  title: "Discovery",
  description: "Share your needs and preferences."
}, {
  step: "02",
  title: "Curated Tours",
  description: "View hand-selected properties."
}, {
  step: "03",
  title: "Application",
  description: "Expert guidance through paperwork."
}, {
  step: "04",
  title: "Move-In",
  description: "Seamless closing coordination."
}];
  ...
  return <div className="min-h-screen">
      ...
      <section className="relative min-h-[40vh] sm:min-h-[45vh] md:min-h-[55vh] lg:min-h-[70vh] flex items-center justify-center pt-16 sm:pt-20 md:pt-28 lg:pt-32 xl:pt-40 pb-6 sm:pb-8 md:pb-12 lg:pb-16 xl:pb-24">
        ...
        <div className="container mx-auto px-4 md:px-6 max-w-4xl text-center relative z-10">
          ...
          <p className="text-sm sm:text-base md:text-lg lg:text-xl text-muted-foreground font-light animate-fade-in max-w-2xl mx-auto mb-5 sm:mb-6 md:mb-8" style={{
          ...
          <div className="flex flex-col gap-4 max-w-md mx-auto animate-fade-in" style={{
          ...
          {/* StreetEasy Link */}
          <a href="https://streeteasy.com/profile/957575-bridge-advisory-group?tab_profile=active_listings" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-accent hover:text-accent/80 font-light transition-colors animate-fade-in text-sm md:text-base mt-6" style={{
      ...
      <section className="py-12 md:py-20 lg:py-28 border-b border-white/5 bg-white/[0.01]" ref={portfolioReveal.elementRef}>
        <div className="container mx-auto px-4 md:px-6 max-w-6xl">
          <div className={`transition-all duration-700 ${portfolioReveal.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
              {buildingsLoading ? Array.from({
              length: 4
            }).map((_, index) => <div key={index} className="p-6 rounded-lg border border-white/10 bg-white/[0.02]">
                    <Skeleton className="h-5 w-32 mb-2" />
                    <Skeleton className="h-4 w-24 mb-3" />
                    <Skeleton className="h-4 w-16" />
                  </div>) : buildings?.slice(0, 4).map((building, index) => <div key={building.id} className="p-6 rounded-lg border border-white/10 bg-white/[0.02] hover:bg-white/[0.04] transition-all" style={{
              transitionDelay: `${index * 100}ms`
                    ...
                    {building.unit_count && <p className="text-accent text-sm font-medium">{building.unit_count} units</p>}
                  </div>)}
            </div>
      ...
      <section className="py-12 md:py-20 lg:py-28 border-b border-white/5" ref={listingsReveal.elementRef}>
        <div className="container mx-auto px-4 md:px-6 max-w-5xl">
          <div className={`text-center transition-all duration-700 ${listingsReveal.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <div className="container mx-auto px-4 md:px-6 max-w-5xl">
          <div className={`transition-all duration-700 ${marketsReveal.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {markets.map((market, index) => <div key={market.borough} className="rounded-lg border border-white/10 bg-white/[0.02] overflow-hidden transition-all duration-300 hover:bg-white/[0.04]" style={{
              transitionDelay: `${index * 100}ms`
            }}>
                  <div className="aspect-[4/3] overflow-hidden">
                    <img src={market.image} alt={market.borough} className="w-full h-full object-cover transition-transform duration-500 hover:scale-105" />
                  </div>
                  <div className="p-6">
                    <h3 className="text-xl font-light mb-3">{market.borough}</h3>
                    <div className="flex flex-wrap gap-2">
                      {market.areas.map(area => <span key={area} className="text-xs text-muted-foreground bg-white/5 px-2 py-1 rounded">
                          {area}
                        </span>)}
                    </div>
                  </div>
      ...
      <section className="py-12 md:py-20 lg:py-28 border-b border-white/5 bg-white/[0.01]" ref={processReveal.elementRef}>
        <div className="container mx-auto px-4 md:px-6 max-w-5xl">
          <div className={`transition-all duration-700 ${processReveal.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
              {processSteps.map((item, index) => <div key={item.step} className="p-6 rounded-lg border border-white/10 bg-white/[0.02] text-center" style={{
              transitionDelay: `${index * 100}ms`
            }}>
