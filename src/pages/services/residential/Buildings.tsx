...
export default function ResidentialBuildings() {
  ...
  const heroContent = <section className="relative bg-gradient-to-b from-secondary to-background pt-32 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <p className="text-primary font-medium mb-4">Residential / Exclusive Portfolio </p>
  ...
  return <ServicePageLayout serviceKey="residential" heroContent={heroContent}>
      ...
      <section className="py-12 md:py-20 lg:py-28" ref={gridReveal.elementRef}>
        <div className="container mx-auto px-4 md:px-6 max-w-6xl">
          <div className={`transition-all duration-700 ${gridReveal.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            {isLoading ? <div className="space-y-12">
                {[1, 2].map(i => <div key={i}>
                ...
                {Object.entries(buildingsByBorough).map(([borough, boroughBuildings], boroughIndex) => <div key={borough}>
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                      {boroughBuildings?.map((building, index) => <div key={building.id} className="group p-6 rounded-lg border border-border/50 bg-card hover:border-accent/30 transition-all duration-300" style={{
                  transitionDelay: `${(boroughIndex * 3 + index) * 50}ms`
                }}>
                          <div className="flex items-start justify-between mb-3">
                            <Building2 className="h-5 w-5 text-accent" />
                            {building.unit_count && <span className="text-xs text-accent bg-accent/10 px-2 py-1 rounded">
                          ...
                          {building.neighborhood && <p className="text-sm text-muted-foreground font-light mb-2">
                              {building.neighborhood}
                            </p>}
                          <p className="text-xs text-muted-foreground/70 font-light">
                            {building.address}
                          </p>
                          {building.tags && building.tags.length > 0 && <div className="flex flex-wrap gap-1 mt-3">
                              {building.tags.slice(0, 3).map(tag => <span key={tag} className="text-xs text-muted-foreground bg-secondary px-2 py-0.5 rounded">
                                  {tag}
                                </span>)}
                            </div>}
                        </div>)}
