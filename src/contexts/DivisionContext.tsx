...
export const DivisionProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Fetch user's assigned division from database for non-admin users
  const { data: userDivisionData, isLoading: divisionLoading } = useQuery({
    queryKey: ["userAssignedDivision"],
    queryFn: async () => {
  ...
  // Admin's selected division (stored in localStorage for UX only - not security)
  ...
  // Sync localStorage when admin changes division
  useEffect(() => {
    if (isAdmin) {
      localStorage.setItem("admin-division-preference", adminSelectedDivision);
    }
  }, [adminSelectedDivision, isAdmin]);

  // Determine actual division to use
  const division: Division = isAdmin 
    ? adminSelectedDivision 
    : (userDivisionData || "investment-sales");

  const hasDivisionAssigned = isAdmin || userDivisionData !== null;
