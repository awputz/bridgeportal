...
export const TeamMemberDialog = ({
  ...
  const { data: investmentListings = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || [])
        .map((item: any) => item.investment_listings as InvestmentListing)
        .filter((listing: InvestmentListing) => listing?.is_active);
    },
    enabled: open && !!member?.id,
  });

  // Fetch agent's current commercial listings
  const { data: commercialListings = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Filter for active listings and flatten
      return (data || [])
        .map((item: any) => item.commercial_listings as CommercialListing)
        .filter((listing: CommercialListing) => listing?.is_active);
    },
    enabled: open && !!member?.id,
  });
  ...
  const ContentComponent = ({ onClose }: { onClose?: () => void }) => (
          ...
          {socialLinks.map(link => {
            ...
            return (
              <a
                ...
                className="flex items-center justify-center h-11 w-11 rounded-md border border-border/40 hover:border-accent/50 hover:bg-accent/10 transition-all duration-300 touch-manipulation active:scale-95"
      ...
      <Tabs defaultValue="exclusives" className="animate-in fade-in slide-in-from-bottom-4 duration-300 ease-out delay-150">
        ...
        <TabsContent value="exclusives" className="mt-4">
          {totalExclusives === 0 ? (
            ...
            <ScrollArea className="h-[280px] md:h-[320px] pr-4">
              <div className="grid gap-3">
                ...
                {investmentListings.map((listing) => (
                  <Link
                    ...
                    onClick={() => onClose?.()}
                    className="group flex gap-4 p-3 rounded-lg border border-border/30 hover:border-accent/50 hover:bg-accent/5 transition-all duration-200"
                  >
      ...
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content
          className={cn(
            ...
            "w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6 md:p-8 bg-background/95 backdrop-blur-xl border border-border/50 shadow-2xl rounded-xl",
