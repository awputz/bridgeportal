...
export const TeamMemberDialog = ({
  ...
  return <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content className={cn("fixed left-[50%] top-[50%] z-50 translate-x-[-50%] translate-y-[-50%]", "w-full max-w-2xl p-8 md:p-12 bg-background/95 backdrop-blur-xl border border-border/50 shadow-2xl rounded-lg", "data-[state=open]:animate-in data-[state=closed]:animate-out", "data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0", "data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-100", "data-[state=open]:slide-in-from-bottom-2", "transition-all duration-300 ease-out")}>
        ...
        <div className="space-y-8">
          ...
          {/* Bio */}
          {member.bio && <p className="text-base leading-relaxed text-muted-foreground/90 animate-in fade-in slide-in-from-bottom-4 duration-300 ease-out delay-75">
              {member.bio}
            </p>}
          ...
          <div className="flex flex-wrap items-center gap-3 pt-4 animate-in fade-in slide-in-from-bottom-4 duration-300 ease-out delay-100">
            ...
            {contactActions.map(action => {
              ...
              return <a key={action.label} href={action.url} target={isExternal ? "_blank" : undefined} rel={isExternal ? "noopener noreferrer" : undefined} className="flex items-center gap-2 px-4 py-2 rounded-lg border border-border/30 hover:border-accent/50 hover:bg-accent/10 transition-all duration-300">
