...
interface UserWithRole {
  ...
  roles: Array<{ role: AppRole; assigned_division: string | null }>;
}
...
export function useUserRolesAdmin() {
  ...
  const { data: users, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Get all user roles with assigned_division
      ...
      const usersWithRoles: UserWithRole[] = (profiles || []).map((profile) => ({
        ...
        roles: (roles || [])
          ...
          .map((r) => ({ role: r.role as AppRole, assigned_division: r.assigned_division })),
      }));
